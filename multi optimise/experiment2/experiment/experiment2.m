...
% 1. 配置目标层（可灵活修改）
target_layers = [1,3,5,7]; % 你的需求：第2、4层
...
for idx = 1:length(target_layers)
    ...
    k_start = find(ra2_ex >= layer_depth_start, 1, 'first');
    k_end = find(ra2_ex <= layer_depth_end, 1, 'last');
    if isempty(k_start); k_start = length(ra2_ex); end
    if isempty(k_end); k_end = 1; end
    k_start = max(1, k_start);
    k_end = min(length(ra2_ex), k_end);
...
for i = 1:numSubSwarms
    for j = 1:popSize
        ...
        % 计算初始适应度
        fitness = sqrt(mean((convolution2(fit, subSwarmPositions(j, 1), subSwarmPositions(j, 2), subSwarmPositions(j, 3), subSwarmPositions(j, 4), subSwarmPositions(j, 5)) - fit).^2));
...
plot(valid_indices, fit(valid_indices), 'bs', 'MarkerSize', 6, 'DisplayName', 'Target Layer Input');
