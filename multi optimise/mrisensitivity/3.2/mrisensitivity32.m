...
%% ============== 步骤2：Morris筛选法配置（核心修改2：分析MRI 4个参数，新增sigema_k） ==============
% 2.1 定义待分析的MRI参数信息（4个参数，名称+上下界，修正sigmea_k为sigema_k）
numSensParams = 4; % 从3个改为4个（新增sigema_k）
sensParamNames = {'w', 'lambda', 'sigema_0', 'sigema_k'}; % 统一命名，修正拼写
% MRI参数下界（参考原始值+DefineDomain，补充sigema_k下界）
sensParamLower = [0.5, 0.5, 0.5, 0.0]; % sigema_k下界：0.0（参考DefineDomain）
% MRI参数上界（参考原始值+DefineDomain，补充sigema_k上界）
sensParamUpper = [3, 3, 3, 1.0]; % sigema_k上界：1.0（参考DefineDomain）
% Morris实验设计参数（不变，平衡计算量和稳定性）
numTrajectories = 4; % 轨迹数
numLevels = 6; % 每个参数的等间距水平数
delta = numLevels / (2 * (numLevels - 1)); % Morris设计核心参数

% 2.2 生成Morris样本集（4维，对应MRI 4个参数）
...
%% ============== 步骤3：批量运行CPSO（核心修改3：固定CPSO参数，遍历MRI 4个参数样本） ==============
fprintf('\nStarting batch CPSO run for MRI parameters (%d samples total)...\n', numSamples);
for sampleIdx = 1:numSamples
    % 提取当前Morris样本的MRI 4个参数（新增sigema_k，从第4列提取）
    ...
    currentMRI_sigmak = morrisSamples(sampleIdx, 4); % 新增：提取sigema_k样本值
    ...
    % 粒子初始化（使用当前MRI样本参数作为初始值参考，新增sigema_k初始化）
    for i = 1:fixedCPSO_numSubSwarms
        for j = 1:fixedCPSO_popSize
            % 初始化MRI核心参数（从Morris样本取值，新增sigema_k）
            ...
            subSwarmPositions(j, 4, i) = currentMRI_sigmak + randn(1)*0.01; % 新增：sigema_k（小扰动，避免超出范围）
            % 固定sigema_c（无变化）
...
%% ============== 步骤4：计算Morris灵敏度指数（核心修改4：对应MRI 4个参数） ==============
...
%% ============== 步骤5：灵敏度分析可视化（核心修改5：适配MRI 4个参数） ==============
% 5.1 图1：主效应vs交互效应散点图（自动适配4个参数）
figure('Position', [100, 100, 800, 600]);
scatter(mu, sigma, 200, mu_star, 'filled', 'MarkerEdgeColor', 'k');
xlabel('Main Effect (\mu)');
ylabel('Interaction Effect (\sigma)');
title('Morris Global Sensitivity Analysis: MRI Core Parameters (Including sigema_k)');
cbar = colorbar;
...
title('MRI Parameters Sensitivity Ranking (Total Effect, Including sigema_k)');
grid on;
% 标注数值
for i = 1:numSensParams
...
% 5.3 图3：MRI参数的RMSE影响趋势图（4个子图，1行4列，适配新增sigema_k）
figure('Position', [300, 300, 1600, 400]);
layoutRow = 1; % 1行
layoutCol = 4; % 4列（适配4个MRI参数，完美容纳）
...
% 调整子图间距，添加总标题
sgtitle('RMSE Influence Trend of MRI Core Parameters (Including sigema_k, Sorted by Sensitivity)', 'FontSize', 12, 'FontWeight', 'bold');
set(gcf, 'Color', 'w');

%% ============== 步骤6：图3数据拆分，保存到MATLAB工作区（适配4个参数，新增sigema_k变量） ==============
fprintf('\nCreating separate variables for Figure 3 in MATLAB workspace...\n');

% 遍历4个MRI参数，在工作区创建独立变量（新增sigema_k）
for idx = 1:numSensParams
    % 1. 提取当前参数信息
    paramName = sensitivityResult.MRI_ParamName{idx}; % 当前参数名称（含sigema_k）
    ...
    % 横坐标变量：如 w_x, lambda_x, sigema_0_x, sigema_k_x
    x_var_name = sprintf('%s_x', paramName);
    assignin('base', x_var_name, param_x_data);
    
    % 纵坐标变量：如 w_y, lambda_y, sigema_0_y, sigema_k_y（你重点需要的RMSE数据）
    y_var_name = sprintf('%s_y', paramName);
    assignin('base', y_var_name, param_y_data);
    
    % （可选）创建趋势线数据变量：如 w_trend, lambda_trend, sigema_0_trend, sigema_k_trend
...
fprintf('All Figure 3 data variables (including sigema_k) are ready in MATLAB workspace!\n');

%% ============== 步骤7：总耗时输出 ==============
fprintf('\nTotal runtime: %.2fs\n', toc());

%% ============== 辅助函数：卷积函数（确保代码可独立运行，无外部依赖） ==============
